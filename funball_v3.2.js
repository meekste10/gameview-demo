/* FunBall v3.2 — Dual-View + Panning Top-Down + Zooming 3D Follow Cam */
(()=>{
// ---------- Constants ----------
...
const PAD = { x: 70, y: 40 };
const TRAIL_MAX_XY = 40;
const TRAIL_MAX_3D = 60;
...
function frameDt(i){ if(i<=0) return 1/fps;
  const t0=frames[i-1]?.t??((i-1)/fps), t1=frames[i]?.t??(i/fps);
  return Math.max(1e-6,t1-t0); }

function sizeCanvas(c){
  const dpr=window.devicePixelRatio||1;
  const cssW=c.clientWidth||c.width, cssH=c.clientHeight||c.height;
  c.width=Math.round(cssW*dpr); c.height=Math.round(cssH*dpr);
  const ctx=c.getContext('2d'); ctx.setTransform(dpr,0,0,dpr,0,0);
}
function resizeAll(){
  [cvXY,cv3D].forEach(sizeCanvas);
  if(typeof OffscreenCanvas==='function'){ bufXY=new OffscreenCanvas(cvXY.width,cvXY.height);
    buf3D=new OffscreenCanvas(cv3D.width,cv3D.height); }
  else { bufXY=document.createElement('canvas'); buf3D=document.createElement('canvas');
...
function normalizeDataset(raw){
  ...
  for(const f of samp){const pts=[];
    if(f.ball)pts.push(f.ball); if(Array.isArray(f.players))pts.push(...f.players);
  ...
  function toWorldLateral(vRaw){const v=toYd(vRaw);
    return clamp(v-FIELD_WID_YD/2,-FIELD_WID_YD/2,FIELD_WID_YD/2);}
}

// ---------- 3D Projection ----------
let BASE_SCALE_D=0.45,BASE_SCALE_L=0.35,Z_PX=12,TILT=0.55;
function refresh3DConstants(){
  const H=cv3D.height,W=cv3D.width;
  BASE_SCALE_D=clamp(W/600,0.35,0.6);
  BASE_SCALE_L=clamp(H/400,0.25,0.45);
  Z_PX=clamp(H/40,6,14);
  TILT=clamp(H/W,0.40,0.70);
}
function map3D_worldToCanvas(p){
  const dy=(p.y-camera.y),dx=(p.x-camera.x);
  const horizonX=cv3D.width*0.5,horizonY=cv3D.height*0.7;
  const scaleD=BASE_SCALE_D*cv3D.width*camera.zoom;
  const scaleL=BASE_SCALE_L*cv3D.height*camera.zoom;
...
// ---------- Field Background ----------
function drawStatic(){
  // base field pattern for reference
  ...
  bxy.strokeStyle='rgba(255,255,255,.2)';
  bxy.beginPath();
...
// ---------- Playback ----------
function step(n=1){ if(!frames.length)return;
  k=Math.max(0,Math.min(frames.length-1,k+n)); drawFrame(k); }
function loop(t){
  if(!playing){lastT=t;requestAnimationFrame(loop);return;}
  if(!lastT)lastT=t; let dt=t-lastT; lastT=t; acc+=dt;
  while(acc>=tickMS){step(1);acc-=tickMS;} requestAnimationFrame(loop);
}
...
function drawFrame(i){
  ...
  let speedYdPerSec=0;
  if(i>0){const prev=frames[i-1];const dt=frameDt(i);
    const d=Math.hypot(f.ball.x-prev.ball.x,f.ball.y-prev.ball.y);
    speedYdPerSec=d/dt;}
  const speedClamped=clamp(speedYdPerSec,0,14);
  const targetZoom=map(speedClamped,0,14,1.1,0.8);
  camera.zoom=lerp(camera.zoom,targetZoom,0.08);

  // --- Top-Down panning / zoom ---
  if(!camera.xy)camera.xy={x:f.ball.x,y:f.ball.y,zoom:1};
  const camXY=camera.xy;
  camXY.x=lerp(camXY.x,f.ball.x,0.15);
  camXY.y=lerp(camXY.y,f.ball.y,0.15);
  let speedXY=0;
  if(i>0){const prev=frames[i-1];
    const dt=frameDt(i);const d=Math.hypot(f.ball.x-prev.ball.x,f.ball.y-prev.ball.y);
    speedXY=d/dt;}
  const targetZoomXY=map(clamp(speedXY,0,14),0,14,1.05,0.85);
  camXY.zoom=lerp(camXY.zoom,targetZoomXY,0.08);

  cxXY.clearRect(0,0,cvXY.width,cvXY.height);
  const w=cvXY.width,h=cvXY.height;
  const scaleX=(w/FIELD_WID_YD)*camXY.zoom, scaleY=(h/FIELD_LEN_YD)*camXY.zoom;

  // move grid lines with camera
  const gridStep=10; // yards per major line
  const offsetY=(camXY.y%gridStep)*scaleY;
  const offsetX=(camXY.x%gridStep)*scaleX;
  cxXY.fillStyle='#0b4a18'; cxXY.fillRect(0,0,w,h);
  cxXY.strokeStyle='rgba(255,255,255,.15)'; cxXY.lineWidth=1;
  cxXY.beginPath();
  for(let gy=-FIELD_LEN_YD/2;gy<FIELD_LEN_YD/2;gy+=gridStep){
    const y=h/2-(gy-(f.ball.y-camXY.y))*scaleY; cxXY.moveTo(0,y); cxXY.lineTo(w,y);
  }
  for(let gx=-FIELD_WID_YD/2;gx<FIELD_WID_YD/2;gx+=gridStep){
    const x=w/2+(gx-(f.ball.x-camXY.x))*scaleX; cxXY.moveTo(x,0); cxXY.lineTo(x,h);
  }
  cxXY.stroke();
  cxXY.strokeStyle='rgba(255,255,255,.5)';
  cxXY.strokeRect(PAD.x,PAD.y,w-PAD.x*2,h-PAD.y*2);

  function worldToXY(p){
    const dx=(p.x-camXY.x),dy=(p.y-camXY.y);
    return {x:w/2+dx*scaleX, y:h/2-dy*scaleY};
  }
  ...
  hudXY.textContent=`Frame ${i+1}/${frames.length} • Ball y=${f.ball.y.toFixed(1)} x=${f.ball.x.toFixed(1)} • Zoom=${camXY.zoom.toFixed(2)}`;
  ...
  for(let j=1;j<trail3D.length;j++){
    ...
    cx3D.strokeStyle=`rgba(255,255,255,${0.08*alpha})`;
    cx3D.lineWidth=2;
    cx3D.beginPath(); cx3D.moveTo(ma.x,ma.y); cx3D.lineTo(mb.x,mb.y); cx3D.stroke();
  }
  cx3D.strokeStyle='rgba(255,255,255,0.1)';
...
function loadJSON(url){
  ...
  .then(raw=>{
    ...
    camera.x=frames[0].ball.x; camera.y=Math.max(0,frames[0].ball.y-8); camera.zoom=1;
    resizeAll();
    hudXY.textContent=`✅ Loaded ${frames.length} frame(s) @ ${fps} fps — tap ▶`;
    hud3D.textContent='Follow: Ball'; drawFrame(0);
  }).catch(err=>{
    console.error(err);
    hudXY.textContent=`❌ ${err.message}`; hud3D.textContent=`❌ ${err.message}`;
  });
}
